...
def run_particle_filter():
    ...
    #filt = particle(1000000, filter_y, pphi, ppsi)
...
def lln_and_clt(): 
    x = np.random.rand(20)
    rolling_means = np.zeros(20)
    for i in range(20):
        rolling_means[i] = np.mean(x[:i+1])

    #print(x[0:5]) 
    #print(rolling_means[0:5])

    plt.plot(range(1, 21), rolling_means, label='Rolling Mean')
    ...
    N_sim = 100000
    n = 1
    X = np.random.rand(N_sim, n)
    X_bar = np.sqrt(n) * ((np.mean(X, axis=1) - 0.5) / np.sqrt(1/12))
    plt.hist(X_bar, bins=50, density=True, color='skyblue', edgecolor='black')
    plt.plot(np.linspace(-3,3,100), norm.pdf(np.linspace(-3,3,100)), color='darkblue', lw=2, label='Standard Normal PDF')
    plt.xlabel('Standardized Sample Mean')
    plt.ylabel('Density')
    plt.title('Sampling Distribution when n = 1')
    plt.show()
    ...
    plt.xlabel('Standardized Sample Mean')
    plt.ylabel('Density')
    plt.title('Sampling Distribution when n = 3')
    plt.show()
    ...
    plt.xlabel('Standardized Sample Mean')
    plt.ylabel('Density')
    plt.title('Sampling Distribution when n = 10')
    plt.show()
    
    # \hat\theta_1 = max{X_1, ..., X_n} 
    # \hat\theta_2 = 2 * \bar{X}
    #theta_hat_1 = np.max(x)
    #theta_hat_2 = 2 * np.mean(x)

    #print(theta_hat_1)
    #print(theta_hat_2)

    #def irwin_hall_pdf(t, n): 
    #    t_floor = np.floor(t) 
    #    k_sequence = np.arange(0, t_floor + 1)
    #    s_sequence = (1 / math.factorial(n)) * (-1)**k_sequence * comb(n, k_sequence) * (t - k_sequence)**(n-1)
    #    return np.sum(s_sequence)
    
    #def irwin_hall_pdf_st(t,n):
    #    s = n * (t / (np.sqrt(12*n)) + 0.5)
    #    J = np.sqrt(n/12)
    #    return J * irwin_hall_pdf(s, n)
    ...
    # Plot for n = 5
    #y1  = np.array([irwin_hall_pdf_st(t, 1) for t in x])
    #y2  = np.array([irwin_hall_pdf_st(t, 2) for t in x])
    #y3  = np.array([irwin_hall_pdf_st(t, 3) for t in x])
    #y10 = np.array([irwin_hall_pdf_st(t, 50) for t in x])
    #y   = np.array([norm.pdf(t) for t in x])
    #plt.plot(x, y1, label=f"Irwin-Hall PDF (n={1})")
    #plt.plot(x, y2, label=f"Irwin-Hall PDF (n={2})")
    #plt.plot(x, y3, label=f"Irwin-Hall PDF (n={3})")
    #plt.plot(x, y10, label=f"Irwin-Hall PDF (n={10})")
